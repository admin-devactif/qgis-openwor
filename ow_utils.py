...
def CleanExp(zLayer, zExp):
    ...
    for i in range(len(tExpression)):
        if zCleanExp != "":
            zCleanExp += " || "
        if tExpression[i] == "Chr$(13)":
            zCleanExp += zRetLigne
        else:
            NbFonctionsIn, NbFonctionsOut = CountCaractere(
                tExpression[i], "("), CountCaractere(tExpression[i], ")")
            if (tExpression[i].find("'") != -1 or tExpression[i].find("\"") != -1):
                if (NbFonctionsIn != NbFonctionsOut):
                    NbFonctionsIn, NbFonctionsOut = 0, 0
            if NbFonctionsIn > 0:
                if (NbFonctionsIn == NbFonctionsOut):
                    zCleanExp += NewExpression(tExpression[i])
                else:
...
def NewExpression(zExpression):
        ...
        if zz.find("+") != -1:
            ...
            for k in range(len(ztempo2)):
                ...
                if zz2.find(",") != -1:
                    ...
                    for k in range(len(ztempo2)):
                        zz3 = NetStrInfos(
                            ztempo2[k], True, True, False, False, ())
                        if zz3 != "obj":
                            zztempo2 += zz3 if zztempo2 == "" else "," + zz3
                        else:
                            break
            ...
            for k in range(len(ztempo2)):
                zz2 = NetStrInfos(ztempo2[k], True, True, False, False, ())
                if zz2 != "obj":
                    zztempo2 += zz2 if zztempo2 == "" else "," + zz2
                else:
                    break
            ztempofunctions += zztempo2
        else:
    ...
    if NbFonctionsOut > NbFonctionsIn:
        ztempofunctions = "(" + ztempofunctions
    if NbFonctionsOut < NbFonctionsIn:
        ztempofunctions = ztempofunctions + ")"
...
def GenericFunction(zQGISFunction, zQGISParamFunction):
    ...
    if zQGISFunction == "Chr$":
        if int(zQGISParamFunction) > 30:
            zRetour = chr(int(zQGISParamFunction[0]))
    elif zQGISFunction == "Asc":
        zRetour = ord(zQGISParamFunction[0])
    return zRetour
...
def NetStrInfos(nStr, tLstrip, tRstrip, tUpper, tTitle, ListCarac):
    for i in range(len(ListCarac)):
        nStr = nStr.replace(ListCarac[i], "")
    if tLstrip:
        nStr = nStr.lstrip()
    if tRstrip:
        nStr = nStr.rstrip()
...
def FixeLibUnits(zUnits, zWidth, zScalezBarWidth):
    if zUnits == 0:
        if zWidth > 1000.0:
            szUnits, zWidth, zScalezBarWidth = " km", zWidth, zScalezBarWidth / 1000  # km, / 1000
        elif zWidth < 0.01:
            szUnits, zWidth, zScalezBarWidth = " mm", zWidth, zScalezBarWidth * 1000  # mm, * 1000
        elif zWidth < 0.1:
            szUnits, zWidth, zScalezBarWidth = " cm", zWidth, zScalezBarWidth * 100  # cm,  * 100
        else:
            szUnits, zWidth, zScalezBarWidth = " m", zWidth, zScalezBarWidth
    elif zUnits == 1:
        if zWidth > 5280.0:
            szUnits, zWidth, ScalezBarWidth = " miles", zWidth / \
                5000, (zScalezBarWidth * 5280) / 5000
        elif zWidth == 5280.0:
            szUnits, zWidth, ScalezBarWidth = " mile", zWidth / \
                5000, (zScalezBarWidth * 5280) / 5000
        elif zWidth < 1.0:
            szUnits, zWidth, ScalezBarWidth = " inches", zWidth * \
                10, (zScalezBarWidth * 10) / 12
        elif zWidth == 1.0:
            szUnits, zWidth, ScalezBarWidth = " foot", zWidth / \
                5000, (zScalezBarWidth * 5280) / 5000
        else:
            szUnits, zWidth, zScalezBarWidth = " feet", zWidth, zScalezBarWidth
    elif zUnits == 2:
        if zWidth == 1.0:
            szUnits, zWidth, ScalezBarWidth = " degree", zWidth, zScalezBarWidth
        else:
            szUnits, zWidth, ScalezBarWidth = " degrees", zWidth, zScalezBarWidth
    else:
        szUnits, zWidth, ScalezBarWidth = " inconnu", zWidth, zScalezBarWidth

    if zScalezBarWidth == 0:
        zScalezBarWidth = 1
