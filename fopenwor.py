...
class Ui_Dialog_OW(object):
    ...
    def FixeREPSearchFILE(self):
        ...
        inputDir = QFileDialog.getExistingDirectory(
            self, "Sélectionner le dossier pour la recherche", InitDir, QFileDialog.ShowDirsOnly | QFileDialog.DontResolveSymlinks)
    ...
    def DoREPSearchFILE(self):
        ...
        if zRep == "":
            QMessageBox.information(
                None, "Fonction de recherche", "Aucun dossier à analyser !")
        else :
            if os.path.exists(zRep) :
               ...
               self.labelSearchFILE.setText(
                   str(self.SearchFILE.count()) + " résultat(s).")
...
def MajCtrlButtonMap(self):
        ...
        if len(tMap)==0 :
           ...
           self.labelMap.setText("Carte à représenter :") if len(
               tMap) == 1 else self.labelMap.setText("Cartes à représenter :")
...
def MakeHTMLView(self, tempLayer, subtempLayer):
    ...
    subtempLayer += "<br><font color='#0000ff'><h3>Fenêtres données (tBrowse) : " + str(
    ...
    subtempLayer += "<br><font color='#0000ff'><h3>Sélections (tSelect) : " + str(
...
def MakeTreeView(self):
    ...
    parentItem = MakeParentItem(
        self, "Fenêtres données (tBrowse) : " + str(len(tBrowse)))
    ...
    parentItem = MakeParentItem(
        self, "Sélections (tSelect) : " + str(len(tSelect)))
                  ...
                  try:
                      ...
                      if os.path.exists(zCible):
                         ...
                         vLayer = self.iface.addVectorLayer(
                             uLayer[cLayer], cLayer + " (Données Couche Logique)", "ogr")
                         ...
                         RestoreSETTINGS(zProjectionSetting, zProjectionCRSValue)
                          ...
                          # si pas de DAT, on créé une couche vide ...
                          zFeatureCount = 0
