...
tLine = {'1': 'NoPen', '2': 'SolidLine', '3': 'DashLine',
         '4': 'DotLine', '5': 'DashDotLine', '6': 'DashDotDotLine'}

tLineV2 = {'1': 'no', '2': 'solid', '3': 'dash', '4': 'dot',
           '5': 'dash dot', '6': 'dash dot dot', '7': 'dash'}
...
def Symbo2Vector(self, zLayer, sSymboLayer, nSizeMap, nSizeMapUnits, zType, nForceUnitsMap, nFixeUniProj, zForceCRS, zForceZoom):
    ...
    HasDefautLayerStyle, URIDefautLayerStyle = DefautLayerStyle(
        self, zLayer.source())
    for i in range(len(tSymbo)):
        sstring = NetStrInfos(tSymbo[i], False, True, False, False, ())
        if sstring.upper().find("DISPLAY GRAPHIC") != -1:
            ...
            if not HasDefautLayerStyle:
                ...
                if zLayer.geometryType() == QGis.Point:
                    ...
                    zSymbol = MakeBRUSHV2(
                        zDisplayGraphic, zLayer, symbol, astyle, False)
        elif sstring.upper().find("ZOOM (") != -1 and sstring.upper().find("VISIBILITY ZOOM (") == -1 and sstring.upper().find("OFF") == -1 and not zForceZoom:
            FixeZOOM(sstring, zLayer, nSizeMap, nSizeMapUnits, 4, 1, 2, False)
        elif sstring.upper().find('ALPHA') != -1:
            FixeALPHA(sstring, zLayer, zSymbol)
        elif sstring.upper().find('LABEL ') != -1:
            pass
    MakeLabels(self, zLayer, sSymboLayer, nSizeMap, nSizeMapUnits,
               zType, nForceUnitsMap, nFixeUniProj, zForceCRS, zForceZoom)
...
def SymboRaster(self, zLayer, sSymboLayer, nSizeMap, nSizeMapUnits, zForceZoom):
    ...
    for i in range(len(tSymbo)):
        sstring = NetStrInfos(tSymbo[i], False, True, False, False, ())
        if sstring.upper().find('VISIBILITY ZOOM') != -1 and not zForceZoom:
            FixeZOOM(sstring, zLayer, nSizeMap, nSizeMapUnits, 5, 2, 3, True)
        elif sstring.upper().find("ZOOM (") != -1 and sstring.upper().find("OFF") == -1 and not zForceZoom:
            FixeZOOM(sstring, zLayer, nSizeMap, nSizeMapUnits, 4, 1, 2, False)
        elif sstring.upper().find('ALPHA') != -1:
            FixeALPHA(sstring, zLayer, None)
    return
...
def ValueTypeAna(sInfosMap, iSymboLayer):
    ...
    if tSymboAna[iSymboLayer].upper().find("IGNORE") != -1:
        nTypeAna = str(tTypeAna[6])
    else:
        nTypeAna = str(tTypeAna[4].upper())
    return nTypeAna
    ...
    if zIndexField != -1:
        ...
        if nTypeAna == "VALUES" or nTypeAna == "RANGES":
            ...
            for i in range(1, len(tInfosAna)-1, 1):
                zStr = NetStrInfos(
                    str(tInfosAna[i]), False, False, False, False, ("\"", "'"))
                ...
                if zStr.upper().find("DEFAULT") == -1:
                    ...
                    if zValue != "":
                        ...
                        if nTypeAna == "VALUES":
                            ...
                            MakeAna.updateRangeLabel(
                                0, str(float(zTempoD[0])) + " - "+str(float(zTempoD[1])))
            ...
            ret = prCentro.addAttributes(
                [QgsField("Id", QVariant.Int), QgsField("Valeur", QVariant.Double)])
            ...
            if zIndex != -1:
                zMethode = NetStrInfos(
                    str(txInfos[zIndex]), False, False, False, False, ("'", "\""))
            else:
                zMethode = "CONST"
            ...
            if zIndex != -1:
                nSize = AdapteRatioMapInfo(
                    float(tTypeAna[zIndex]), False, False)

            nType = "rectangle"
            if tTypeAna[zIndex-2] == "circle":
                nType = "circle"

            nRatio = 0.0
            zIndex = FixeIndex(0, xTypeAna, "DENSITY")
            if zIndex != -1:
                nRatio = float(tTypeAna[zIndex])
            ...
            if zIndex != -1:
                nFillColor = tTypeAna[zIndex]

            zProjectionSetting, zProjectionCRSValue = ChangeSETTINGS(
                self, zLayer)
            inLayer = QgsVectorLayer(str(zLayer.source()),  str(
                CleanName(zLayer.name())),  str(zLayer.dataProvider().name()))
            DefineLayerProj(self, zLayer, inLayer)
            RestoreSETTINGS(zProjectionSetting, zProjectionCRSValue)
            fLayer = randomizePoints(
                self, inLayer, minimum, design, value, tmpMax, nSize, nRatio, nFillColor, nType)
            ...
            zUnits = NetStrInfos(
                str(xTypeAna[zIndex]), False, False, True, False, ("\"", "'"))
            ...
            zLeftRightCenterPos = str(
                xTypeAna[zIndex]) if zIndex != -1 else "CENTER"
            zAboveBelowCenterPos = str(
                xTypeAna[zIndex+1]) if zIndex != -1 else "CENTER"

            if nTypeAna == "HALF":
                if zSens == 1:  # sens trigonométrique
                    if zLeftRightCenterPos == "LEFT":
                        zAngle = 180.0
                    elif zLeftRightCenterPos == "RIGHT":
                        zAngle = -180.0
                    else:
                        if zAboveBelowCenterPos == "ABOVE" or zAboveBelowCenterPos == "CENTER":
                            zAngle = 0.0
                        else:
                            zAngle = 360.0
                else:  # sens horaire
            ...
            if zAboveBelowCenterPos == "ABOVE":
                zPondCoordY = 1.00005
            elif zAboveBelowCenterPos == "BELOW":
                zPondCoordY = 0.99995
            else:
                zPondCoordY = 1.0
            ...
            for k in range(len(tInfosAna)):
                ...
                if xTypeAna.find("BRUSH (") != -1:
                    nBrushStyle.append(xTypeAna)
            ...
            for k in range(nbFields):
                zField = NetStrInfos(
                    tAttributs[k], True, True, False, False, ())
                ...
                if zMethode == "LOG":
                    if tmpMin > 0:
                        tmpMin = math.log(tmpMin)
                    if tmpMax > 0:
                        tmpMax = math.log(tmpMax)
                elif zMethode == "SQRT":
                    if tmpMin > 0:
                        tmpMin = math.sqrt(tmpMin)
                    if tmpMax > 0:
                        tmpMax = math.sqrt(tmpMax)
            ...
            while provider.nextFeature(feat):
                ...
                for iAtt in range(len(nLayers)):
                    ...
                    if zMethode == "LOG":
                        if zValueAttr > 0:
                            zValueAttrM = math.log(zValueAttr)
                    elif zMethode == "SQRT":
                        if zValueAttr > 0:
                            zValueAttrM = math.sqrt(zValueAttr)
                    ...
                    if iAtt == 0:
                        ...
                        if zValueAttr > zMaxAttr:
                            zMaxAttr = zValueAttr

                if zFixed > -1:
                    zSizeCircle = int(zSize) * 2.5
                else:
                    zSizeCircle = int(
                        zSize * (SommeValueAttM / zSommeMaxAttr)) * 2.5
                ...
                if zReproject:
                    zUnits = int(self.iface.mapCanvas(
                ...
                for iAtt in range(len(nLayers)):
                    ...
                    if zValueAttr != 0:
                        ...
                        if zPoints < 1:
                            zPoints = 1
                        zPasAngle = zSens * \
                            (float((zAngle - zPosition) / zPoints))
            ...
            if zFixed == -1:
                ...
                zValue = float(xTypeAna[zIndex])
                if zValue > 1000000:
                    zSize = zValue / (zSize * RatioMapInfo)
                else:
                    zSize = zValue / zSize
                zIndex = FixeIndex(0, xTypeAna, "BY")
                if zIndex != -1:
            ...
            zInverse = -1 if (zLeftRightCenterPos ==
                              "LEFT" or zLeftRightCenterPos == "RIGHT") else 1
            if zAboveBelowCenterPos == "ABOVE":
                zPondCoordY = 1.005
            elif zAboveBelowCenterPos == "BELOW":
                zPondCoordY = 0.995
            ...
            while provider.nextFeature(feat):
                ...
                for iAtt in range(len(nLayers)):
                    ...
                    if zValueAttr != 0:
                        zHauteur = round(
                            ((zValueAttr/SommeValueAtt) * zSizeBar), 2)

                        if nTypeAna == "BAR":
                            if zInverse == -1:
                                Xcoord = float(
                                    geom.centroid().asPoint().x()) + zPointDep
                                Ycoord = zPondCoordY * \
                                ...
                                Xcoord = float(geom.centroid().asPoint(
                                ).x()+zPointDep+(iAtt * zSizeBase))
                                Ycoord = zPondCoordY * \
                                    float(geom.centroid().asPoint().y())
                            ...
                            if zInverse == -1:
                                Xcoord = float(geom.centroid().asPoint().x(
                                )) + zPointDep + (zSignCoordX * LastHauteur)
                                Ycoord = zPondCoordY * \
                                    float(geom.centroid().asPoint().y())
                                MyPoint = zTransform.transform(
                                    QgsPoint(Xcoord, Ycoord))
            ...
            Symbo2Vector(self, zLayer, sSymboLayer, nSizeMap, nSizeMapUnits,
                         zType, nForceUnitsMap, zReproject, zForceCRS, zForceZoom)
            fLayer = zLayer
    else:
...
def SymboRasterMIG(self, zLayer, zURL, sSymboLayer, nAnaMIG, nLabelFieldName, nSizeMap, nSizeMapUnits, sInfosMap, vType, nForceUnitsMap):
    ...
    zCommand = "gdal_grid -zfield " + nLabelFieldName + \
        " -l Us_rain -a invdist:power=4.0:smothing=3.0:radius1=0.0:radius2=0.0:angle=0.0:max_points=10:min_points=5:nodata=0.0 -of GTiff"
    ...
    if os.path.exists(zURL):
        ...
        if QGISVersionID <= 10800:
            ...
            myRampItem = QgsColorRampShader.ColorRampItem(
                zMin, QColor(0, 0, 255), str(zMin))
            ...
            myRampItem = QgsColorRampShader.ColorRampItem(
                2*zInt, QColor(0, 100, 150), str(2*zInt))
            myRampItemList.append(myRampItem)
            myPixel = QgsRasterTransparency.TransparentSingleValuePixel()
            myPixel.min = zInt
            myPixel.max = 2*zInt
            ...
            myRampItem = QgsColorRampShader.ColorRampItem(
                3*zInt, QColor(0, 150, 75), str(3*zInt))
            myRampItemList.append(myRampItem)
            myPixel = QgsRasterTransparency.TransparentSingleValuePixel()
            myPixel.min = 2*zInt
            myPixel.max = 3*zInt
...
def ChangeSETTINGS(self, zLayer):
    ...
    if zLayer == None:
        zProjEPSG = str(self.iface.mapCanvas(
        ).mapRenderer().destinationCrs().authid())
    else:
        zProjEPSG = str(zLayer.crs().authid())
...
def DefineLayerProj(self, zLayer, CibleLayer):
    if zLayer == None:
        ...
        CibleLayer.setCrs(zLayer.crs())
    CibleLayer.updateFieldMap()


def RestoreSETTINGS(zProjectionSetting, zProjectionCRSValue):
    settings = QSettings()
    if zProjectionSetting != "":
        settings.setValue("Projections/defaultBehaviour", zProjectionSetting)
    if zProjectionCRSValue != "":
        settings.setValue("Projections/layerDefaultCrs", zProjectionCRSValue)
...
def MakeLabels(self, zLayer, sSymboLayer, nSizeMap, nSizeMapUnits, zType, nForceUnitsMap, zReproject, zForceCRS, zForceZoom):
    ...
    for i in range(len(tSymbo)):
        ...
        if x.startswith('ACTIVATE'):
            ...
            nLabelFieldName = NetStrInfos(
                sstring, True, True, False, False, ("WITH", "With"))
            if nLabelFieldName.find("Proper$") != -1:
                nLabelFieldName = NetStrInfos(
                    nLabelFieldName, True, True, False, False, ("PROPER$", "Proper$", "(", ")"))
            ...
            if atempostr != "FONT":
                zFontAlignement = zFontAlignement + str(axstyle[5])
            aFontProperties = MakePropertiesFONT(6, str(axstyle[6]), axstyle)
            aFontProperties = NetStrInfos(
                aFontProperties, False, False, False, False, ("(", ")"))
            ...
            if len(aFont) == 5:
                ...
                # InvRGB(long(aFont[len(aFont)-2]))
                zFontColor = int(aFont[len(aFont)-2])
                # InvRGB(long(aFont[len(aFont)-1]))
                zBackColor = int(aFont[len(aFont)-1])
            else:
                # InvRGB(long(aFont[len(aFont)-1]))
                zFontColor = int(aFont[len(aFont)-1])
            ...
            if aEtiquette == "ON" and nLabelFieldName != "":
                ...
                zLayer.setCustomProperty(
                    "labeling/fontUnderline", isFontUnderline)
                zLayer.setCustomProperty("labeling/fontItalic", isFontItalic)
                zLayer.setCustomProperty("labeling/multiLineLabels", "true")

                if (zFontAlignement.find("CENTER")) != -1:
                    zFontAlignement = "1"
                elif (zFontAlignement.find("LEFT")) != -1:
                    zFontAlignement = "0"
                else:
                    zFontAlignement = "2"
                ...
                if tempoTest:
                    zLayer.setCustomProperty("labeling/isExpression", "true")
                else:
                    zLayer.setCustomProperty("labeling/isExpression", "false")
                ...
                if zMakeHalo != -1:
                    bufferColorR, bufferColorG, bufferColorB = StrRGB(
                        zBackColor)
                    zLayer.setCustomProperty(
                        "labeling/bufferColorR", bufferColorR)
                    zLayer.setCustomProperty(
                        "labeling/bufferColorG", bufferColorG)
                    zLayer.setCustomProperty(
                        "labeling/bufferColorB", bufferColorB)
                    zLayer.setCustomProperty(
                        "labeling/bufferSize", str(float(0.1*zFontSize)))
                else:
        ...
        elif x.find('VISIBILITY ZOOM') != -1 and not zForceZoom:
            FixeZOOM(x, zLayer, nSizeMap, nSizeMapUnits, 5, 2, 3, True)
        ...
        # ANNOTATIONS <=> etiquettes personnalisees
            ...
            if zIndexA == -1:
                zIndexA = FixeIndex(0, zZoomPropertiesAnnotations, "ANCHOR")
            else:
                IsCallout = True
            X, Y = -1, -1

            if zIndexA > -1 and IsCallout:
                zCoordXY = NetStrInfos(
                    zZoomPropertiesAnnotations[zIndexA], True, True, False, False, ("(", ")"))
            ...
            if zIndex != -1:
                nProp = NetStrInfos(
                    zZoomPropertiesAnnotations[zIndex], True, True, False, False, ("(", ")"))
                nProp = nProp.split(",")

                zFont = NetStrInfos(
                    nProp[0], True, True, False, True, ("\"", "'"))
                ...
                zForeColorV = InvRGB(int(nProp[3]))
                zForeColor = rgb_to_hex(int(nProp[3]))

                if len(nProp) == 5:
                    zBackColorV = InvRGB(int(nProp[4]))
            ...
            if X != -1 and Y != -1:
                ...
                zDoc.setHtml(str("<font style=\"color:"+str(zForeColor)+"; font-family:'"+str(
                    zFont)+"'; font-size: " + str(zFontSize)+"px\">"+str(zText)+"</font>"))
                textItem.setDocument(zDoc)

    zStr = str(zLayer.name())
    if HasMakeLabeller and zStr.find("Etiquettes") == -1:
        zLayer.setLayerName(
            zLayer.name()+" (Etiquettes : "+nLabelFieldName + ")")
    return
...
def GetValueZoom(nValue, nUnits, nSizeMap, nSizeMapUnits):
    class Length(with_metaclass(MeasureType, object)):
        DEFAULT_UNIT = "m"
    ...
    zEchelle = int(float(sValue) / float(sSizeMap))
    return zEchelle
        ...
        for line in mytable:
            ...
            if debFields and CountFields > 0:
                ...
                if astring.startswith(nFieldName):
                    ...
                    tempo = astring.split(" ")
                    FieldNameCible = NetStrInfos(
                        tempo[0], True, True, False, False, ())
...
def GetWMSInfos(nTableRaster):
    ...
    mytable.closed
    nParamWMS = ("GetMap=\"", "<SRS>", "<Layer>", "wms_version=\"",
...
def ExtractLayers(nStr):
    ...
    while posd > posf:
        posf = nStr.find("<", posd)
        if posf > posd:
            if zLayersWMS.endswith("|"):
                zLayersWMS = zLayersWMS + nStr[posd:posf]
            else:
                zLayersWMS = zLayersWMS + "," + nStr[posd:posf]
        else:
            if zLayersWMS.endswith("|"):
                zLayersWMS = zLayersWMS + 'default'
            else:
                zLayersWMS = zLayersWMS + "," + 'default'
...
def randomizePoints(self, inLayer, minimum, design, value, nMax, nSize, nRatio, nFillColor, nType):
    ...
    if design == self.tr("unstratified"):
        ...
        if inLayer.type() == inLayer.RasterLayer:
            points = simpleRandom(self, int(value), ext, ext.xMinimum(
            ), ext.xMaximum(), ext.yMinimum(), ext.yMaximum())
        else:
            points = vectorRandom(self, int(value), inLayer, ext.xMinimum(
            ), ext.xMaximum(), ext.yMinimum(), ext.yMaximum())
    else:
        points = loopThruPolygons(self, inLayer, value, design, ratio)
    ...
    zTempoInfos = str(nSymbol) + "," + str(nFillColor) + "," + \
        str(RatioMapInfo*nSize) + "," + str(nFillColor)
...
def loopThruPolygons(self, inLayer, numRand, design, ratio):
    ...
    while sProvider.nextFeature(sFeat):
        ...
        sPoints.extend(simpleRandom(self, value, sGeom, sExt.xMinimum(
        ), sExt.xMaximum(), sExt.yMinimum(), sExt.yMaximum()))
        count = count + add
    return sPoints
...
def MakeMARKERV2(zDisplayGraphic, zLayer, zSymbol, zInfosSymbos):
    ...
    if zIndex == -1:
        ...
        while zIndex != -1:
            ...
            zInfos = NetStrInfos(zInfos, False, False,
                                 False, False, ("(", ")"))
            vInfos = zInfos.split(",")
...
def MakeLINEV2(zDisplayGraphic, zLayer, zSymbol, zInfosSymbos):
    ...
    if zIndex != -1:
        zInfos = NetStrInfos(
            zInfosSymbos[zIndex], False, False, False, False, ("(", ")"))
        vInfos = zInfos.split(",")
        zSize = float(vInfos[0])
        zTypeLine = str(vInfos[1])
        zColorRGB = int(vInfos[2])
    ...
    if zTypeLine in tLineCompositeV2:
        ...
        for i in range(len(zSubLine)):
            ...
            if zSubLine[i] == '0':
                ...
                if zTypeLine in tSubSymbLineCompositeV2:
                    ...
                    for k in range(len(zSubSubLine)):
                        if int(zSubSubLine[k]) == i or int(zSubSubLine[k]) == -1:
                            ...
                            if zInfoSubLine[k].split("|")[1] != "%":
                                zSizeLine = str(zInfoSubLine[k].split("|")[1])
                ...
                if zTypeLine in tSubSymbSymbolCompositeV2:
                    TheSubStyles = tSubSymbSymbolCompositeV2[zTypeLine]
                    zQtMarker = str(TheSubStyles[1])
                    if TheSubStyles[2] != "%":
                        zSizeSymbol = str(TheSubStyles[2])
                    else:
                        # AdapteRatioMapInfo(zSize, False, True))
                        zSizeSymbol = str(zSize)
        ...
        zPathLine = os.path.join(
            zPath + "/svg/", "mapinfo_line_"+zLineSTR+".svg")

        if os.path.exists(zPathLine):
            zPathLine = ReColoreSVG(
                zPath, zPathLine, "line", zLineSTR, zColorRGB)
            ...
            if os.path.exists(zPathLine):
                ...
                zSymbol.symbolLayer(
                    indexSubSymbolLayer).subSymbol().deleteSymbolLayer(0)
            else:
                zSymbol = MakeSimpleLINEV2(
                    zSymbol, zColorLine, '0.0', zQtLine, zSizeLine, True)
        else:
            zSymbol = MakeSimpleLINEV2(
                zSymbol, zColorLine, '0.0', zQtLine, zSizeLine, True)
...
def MakeBRUSHV2(zDisplayGraphic, zLayer, zSymbol, zInfosSymbos, zIsAna):
    ...
    if zIndex == -1:  # or zDisplayGraphic
        ...
        while zIndex != -1:
            ...
            if zBrush in tBrushV2:
                zIndexPEN = FixeIndex(0, zInfosSymbos, "PEN")
                if zIndexPEN != -1:
                    zInfos = NetStrInfos(
                        zInfosSymbos[zIndexPEN], False, False, False, False, ("(", ")"))
                    vInfos = zInfos.split(",")
                    ...
                    zSize = "1.0"
                # props {'color_border', 'style_border', 'offset', 'style', 'color', 'width_border'}
                vTempoInfos = zPENColor + "," + zLinePen + \
                    ",1.0," + zBrush + "," + zForeColor + "," + zSize
                ...
                zPathTexture = os.path.join(
                    zPath + "/svg/", "mapinfo_brush_"+zBrushSTR+".svg")

                if os.path.exists(zPathTexture):
                    zPathTexture = ReColoreSVG(
                        zPath, zPathTexture, "brush", zBrushSTR, zForeColor)
                    # props {'color_border', 'style_border', 'offset', 'style', 'color', 'width_border'}
                    zIndexPEN = FixeIndex(0, zInfosSymbos, "PEN")
                    if zIndexPEN != -1:
                        zInfos = NetStrInfos(
...
def MakeSvgMARKERV2(vInfos):
    ...
    zSymbolFileName = NetStrInfos(
        str(vInfos[0]), True, True, False, False, ("\"", "'"))
...
def ReColoreSVG(zPath, zSVGFileIn, zTypeSVG, zValueSVG, zColor):
    ...
    if zColor != '16777215':
        MyColorHexa = rgb_to_hex(int(zColor))
        MyColorHexa = MyColorHexa.replace("#", "")
        zPathTextureColor = os.path.join(
            zPath + "/myworks/", "mapinfo_"+zTypeSVG+"_"+zValueSVG+"_"+str(MyColorHexa)+".svg")
    ...
    if not os.path.exists(zPathTextureColor):
        ...
        for line in mySVG:
            ...
            zStr = zStr.replace(
                "stroke:#000000", "stroke:#"+str(MyColorHexa))+"\n"
...
def FixeZOOM(zInfos, zLayer, zSizeMap, zSizeMapUnits, indexUnitsZoom, indexValueMinZoom, indexValueMaxZoom, IsZoomLabels):
    ...
    zUnitsZoom = NetStrInfos(
        zZoomProperties[indexUnitsZoom], False, False, True, False, ("\"", "'"))
    zValueMinZoom = NetStrInfos(
        zZoomProperties[indexValueMinZoom], True, True, False, False, ("(", ","))
    zValueMinZoom = float(GetValueZoom(
        float(zValueMinZoom), zUnitsZoom, zSizeMap, zSizeMapUnits))
    zValueMaxZoom = NetStrInfos(
        zZoomProperties[indexValueMaxZoom], False, False, False, False, (")"))
    zValueMaxZoom = float(GetValueZoom(
        float(zValueMaxZoom), zUnitsZoom, zSizeMap, zSizeMapUnits))
...
def FixeALPHA(zInfos, zLayer, zSymbol):
    ...
    if aALPHA > 1.0:
        aALPHA = 1.0
    if zLayer.type() == QgsMapLayer.VectorLayer:
        zSymbol.setAlpha(aALPHA)
    elif zLayer.type() == QgsMapLayer.RasterLayer:
        zLayer.setTransparency(zALPHA)
    return
...
def FixeSize(zSize, zDpi, zUnits):
    if zUnits == "MM":
        zSize = float(zSize * zDpi / 25.4)
    elif zUnits == "CM":
        zSize = float(zSize * zDpi / 2.54)
    return zSize
...
def AdapteRatioMapInfo(zValue, isLINE, isFONT):
    zCoeff = 1
    if float(zValue) > 10.0 and isLINE and not isFONT:
        zCoeff = math.ceil(zValue/10.0) * 2
    if float(zValue) < 10.0 and isLINE and not isFONT:
        zCoeff = 1
    if float(zValue) > 10.0 and not isLINE and isFONT:
        zCoeff = 0.5
...
def MakeComposer(self, iComposer, iInfosComposer):
    ...
    if int(iComposer) == -1:
        ...
        MakeTEXT(self, c, composerView, c.paperWidth()/3, 10.0,
                 "Ma carte MapInfo", QColor(0, 0, 255), -1, dpmm, True)
        MakeLEGEND(self, c, composerView, 10.0, 10.0, "-1",
                   QColor(255, 240, 120), QColor(0, 0, 0), 0.26)
        ...
        for i in range(len(tInfosComposer)-1, -1, -1):
            stempo = NetStrInfos(
                tInfosComposer[i], True, False, False, False, ())
            x = stempo.upper()
        ...
        for i in range(len(tInfosComposer)):
            ...
            if stempo.upper().startswith("SET COORDSYS LAYOUT UNITS"):
                stempo = NetStrInfos(
                    stempo.upper(), True, False, False, False, ("SET COORDSYS LAYOUT UNITS ", "\""))
                zUnit = stempo.lower()
            elif stempo.upper().startswith("CREATE FRAME"):
                i, composerMap = MakeFRAME(
                    self, c, composerView, tInfosComposer, stempo, i, dpmm)
                MakeSCALEBAR(self, c, composerView, composerMap)
            elif stempo.upper().startswith("CREATE ELLIPSE") or stempo.upper().startswith("CREATE RECT"):
                i, composerShape = MakeOBJECT(
                    self, c, composerView, tInfosComposer, stempo, i, dpmm)
            elif stempo.upper().startswith("CREATE LINE"):
                i = MakeARROW(self, c, composerView,
                              tInfosComposer, stempo, i, dpmm)
            elif stempo.upper().startswith("CREATE TEXT"):
                i = MakeTEXT(self, c, composerView, 0.0, 0.0,
                             tInfosComposer, QColor(255, 0, 0), i, dpmm, False)
                i += 1
...
def MakeTEXT(self, c, composerView, posx, posy, zInfos, zColor, zpos, dpmm, isSimpleText):
    ...
    if not isSimpleText:
        zText = NetStrInfos(zInfos[zpos+1], True,
                            False, False, False, ("'", "\""))
        stempo = NetStrInfos(zInfos[zpos+2], True,
                             False, False, False, ("(", ")"))
        ...
        if zIndex != -1:
            aFontProperties = MakePropertiesFONT(
                zIndex, ztempo[zIndex], ztempo)
            aFontProperties = NetStrInfos(
                aFontProperties, False, False, False, False, ("(", ")"))
            aFont = aFontProperties.split(",")
            ...
            if len(aFont) == 5:
                # il y a un fond ou un halo
                zMakeHalo = ValCarac(int(aFont[1]), 2)
                zColor = InvRGB(int(aFont[len(aFont)-2]))
                zBackColor = InvRGB(int(aFont[len(aFont)-1]))
            else:
                zColor = InvRGB(int(aFont[len(aFont)-1]))
            zFontCarac = ValCarac(int(aFont[1]), 0)
            ...
            zFontSize = int(aFont[2])
            zPolice = NetStrInfos(
                aFont[0], True, True, False, True, ("\"", "'"))

        labelFont = MakeFONT(zPolice, zFontSize, zFontBold,
                             zFontItalic, zFontUnderline, False, 0)

    else:
        zText = zInfos
        labelFont = MakeFONT(zPolice, zFontSize, zFontBold,
                             zFontItalic, zFontUnderline, False, 2)
...
def MakeARROW(self, c, composerView, zlistCompo, Rac, wpos, dpmm):
    stempo = NetStrInfos(Rac.upper(), False, False, False,
                         False, ("CREATE LINE ", "(", ")"))
    stempo = stempo.replace(" ", ",")
    tDim = stempo.split(",")
...
def MakeLEGEND(self, c, composerView, posx, posy, zStyleBrush, zFillColor, zLineColor, zLineSize):
    ...
    if zLineSize > 0:
        composerLegend.setFrame(1)
    else:
        composerLegend.setFrame(0)
...
def MakeNORTHARROW(self, c, composerView, composerMap):
    ...
    picNorthArrow = QDir.convertSeparators(QDir.cleanPath(
        QgsApplication.qgisSettingsDirPath() + "/python/plugins/openwor/svg/arrow/north_arrow.svg"))
...
def MakeSCALEBAR(self, c, composerView, composerMap):
    ...
    szUnits, zValueUnits, zToto = FixeLibUnits(
        zUnits, zValueScale, zValueScale)
    ...
    composerScaleBar.setItemPosition(
        10.0, c.paperHeight()-composerScaleBar.height()-25.0)
...
def MakeOBJECT(self, c, composerView, zlistCompo, Rac, wpos, dpmm):
    ...
    if TypeObject == "ELLIPSE":
        composerShape.setShapeType(0)
    elif TypeObject == "RECT":
        composerShape.setShapeType(1)
    elif TypeObject == "CIRCLE":
        composerShape.setShapeType(2)
    else:
        composerShape.setShapeType(0)
        ...
        if mpos > tpos:
            if x.startswith('CREATE') or x.startswith('SET WINDOW FRONTWINDOW() PRINTER') or x.startswith('LAYOUT') or x.startswith('SET LEGEND') or x.startswith('SET WINDOW FRONTWINDOW() TITLE') or x.startswith('BROWSE *') or x.startswith('MAP FROM '):
                ...
                if x.startswith("PEN"):
                    ...
                    zInfos = NetStrInfos(
                        x, False, False, False, False, ("BRUSH (", ")"))
                    vInfos = zInfos.split(",")
                    zBrush = str(vInfos[0])
                    zFillColor = InvRGB(int(vInfos[1]))
...
def MakeFRAME(self, c, composerView, zlistCompo, Rac, wpos, dpmm):
        ...
        if mpos > tpos:
            if x.startswith('CREATE') or x.startswith('SET WINDOW FRONTWINDOW() PRINTER') or x.startswith('LAYOUT') or x.startswith('SET LEGEND') or x.startswith('SET WINDOW FRONTWINDOW() TITLE') or x.startswith('BROWSE *') or x.startswith('MAP FROM '):
                ...
                if x.find('LÉGENDE') != -1:
                    ...
                    nSTR = NetStrInfos(astring, True, False,
                                       False, False, ("Title ", "'"))
                    nSTR = nSTR.split(" ")
                ...
                if x.startswith("BRUSH"):
                    ...
                    vInfos = zInfos.split(",")
                    zBrush = str(vInfos[0])
                    zFillColor = InvRGB(int(vInfos[2]))  # 1
                if x.startswith("PEN"):
                    zInfos = NetStrInfos(
                        x, False, False, False, False, ("PEN (", ")"))
                    vInfos = zInfos.split(",")
    ...
    if not isLegend and not isAttributTable:
        ...
        if isLegend:
            MakeLEGEND(self, c, composerView, posx, posy,
                       zBrush, zFillColor, zLineColor, zLineSize)
        if isAttributTable:
            MakeTABLE(self, c, composerView, posx, posy, w,
                      h, zLayerName, zFillColor, zLineColor)
...
def DefautLayerStyle(self, uLayer):
    ...
    URIDefautLayerStyle = QDir.convertSeparators(
        QDir.cleanPath(uLayer[0:len(uLayer)-3]+"qml"))
    if not os.path.exists(URIDefautLayerStyle):
        return False, ""
